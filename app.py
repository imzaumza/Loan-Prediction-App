...
st.set_page_config( 
    ...
    page_icon="https://png.pngtree.com/png-clipart/20190614/original/pngtree-loan-icon-png-image_3700428.jpg" 
)
st.set_option('deprecation.showPyplotGlobalUse', False)

st.title("Loan Default Prediction") 
st.markdown("""----""") 
st.subheader("Are you sure your loan applicant is surely going repay the loan?💸. \n Note: This Machine Learning App will help you to make prediction with your decision")
...
with col2: 
    tab1, tab2 = st.tabs(["Aim", "Objective"])
    tab1.write("To borrow money, credit analysis is performed.")
    tab2.write("""
    ...
    Human approval requires extensive hours effort to review each application, 
    however, the company will always seek cost optimization and improve human productivity. 
    This sometimes causes human error and bias, as it’s not practical to digest a large number of applicants considering all the factors involved.
    """)
...
st.subheader("Find the Prediction Results below: ")
...
#pass the features dictionary to make a prediction by using streamlit widget st.button, 
# and once the button is pressed the predict_proba function is invoked:
...
if btn_predict: 
    ...
    if pred[0] < 0.78: 
        ...
        st.success('It is green! The applicant has a high probability to pay the loan back!')
...
'addr_state__AK','addr_state__AL','addr_state__AR','addr_state__AZ','addr_state__CA',
'addr_state__CO','addr_state__CT', 'addr_state__DC','addr_state__DE','addr_state__FL',
'addr_state__GA','addr_state__HI','addr_state__ID','addr_state__IL','addr_state__IN', 
'addr_state__KS','addr_state__KY','addr_state__LA','addr_state__MA','addr_state__MD',
'addr_state__ME','addr_state__MI','addr_state__MN', 'addr_state__MO','addr_state__MS',
'addr_state__MT','addr_state__NC','addr_state__ND','addr_state__NE','addr_state__NH',
'addr_state__NJ', 'addr_state__NM','addr_state__NV','addr_state__NY','addr_state__OH',
'addr_state__OK','addr_state__OR','addr_state__PA','addr_state__RI', 'addr_state__SC',
'addr_state__SD','addr_state__TN','addr_state__TX','addr_state__UT','addr_state__VA',
'addr_state__VT', 'addr_state__WA', 'addr_state__WI','addr_state__WV','addr_state__WY']) 
y = loans[['loan_status']] 
...
pkl.dump(logreg,open("model.p","wb"))



#We display shap results in the main page after the model prediction, and we include two levels of interpretability 
# 1) Local level interpretability at the applicant level, 
# 2) Global model interpretability of the entire model. At the end we guide the user into how to read the shap results:
...
st.write(""" In this chart, blue and red mean the feature value, 
         e.g. annual income blue is a smaller value e.g. 40K USD, and red is a higher value e.g. 100K USD. 
         The width of the bars represents the number of observations on a certain feature value, 
         for example with the annual_inc feature we can see that most of the applicants are within the lower-income or blue area. 
         And on axis x negative SHAP values represent applicants that are likely to churn 
         and the positive values on the right side represent applicants that are likely to pay the loan back. 
         What we are learning from this chart is that features such as annual_inc 
         and sub_grade are the most impactful features driving the outcome prediction. 
         The higher the salary is, or the lower the subgrade is, 
         the more likely the applicant to pay the loan back and vice versa, which makes total sense in our case. 
""")